...
import org.starfire.shine.geom.ShapeRenderer;
import org.starfire.shine.opengl.TextureImpl;
import org.starfire.shine.opengl.renderer.LineStripRenderer;
import org.starfire.shine.opengl.renderer.Renderer;
import org.starfire.shine.opengl.renderer.SGL;
import org.starfire.shine.util.FastTrig;
...
public class Graphics {
	/** The renderer to use for all renderer operations */
	protected static SGL renderer = Renderer.get();
	...
	private GLCapabilities glcap = null;
				...
				public Object run() {
					try {
						DEFAULT_FONT = new AngelCodeFont(
								"org/starfire/shine/data/defaultfont.fnt",
								"org/starfire/shine/data/defaultfont.png");
	...
	public void setDrawMode(int mode) {
		...
		if (currentDrawingMode == MODE_NORMAL) {
			renderer.glEnable(SGL.GL_BLEND);
			renderer.glColorMask(true, true, true, true);
			renderer.glBlendFunc(SGL.GL_SRC_ALPHA, SGL.GL_ONE_MINUS_SRC_ALPHA);
		}
		if (currentDrawingMode == MODE_ALPHA_MAP) {
			renderer.glDisable(SGL.GL_BLEND);
		...
		if (currentDrawingMode == MODE_ALPHA_BLEND) {
			...
			renderer.glBlendFunc(SGL.GL_DST_ALPHA, SGL.GL_ONE_MINUS_DST_ALPHA);
		}
		if (currentDrawingMode == MODE_COLOR_MULTIPLY) {
			renderer.glEnable(SGL.GL_BLEND);
			renderer.glColorMask(true, true, true, true);
			renderer.glBlendFunc(SGL.GL_ONE_MINUS_SRC_COLOR, SGL.GL_SRC_COLOR);
		}
		if (currentDrawingMode == MODE_ADD) {
			renderer.glEnable(SGL.GL_BLEND);
			renderer.glColorMask(true, true, true, true);
			renderer.glBlendFunc(SGL.GL_ONE, SGL.GL_ONE);
		}
		if (currentDrawingMode == MODE_SCREEN) {
			renderer.glEnable(SGL.GL_BLEND);
			renderer.glColorMask(true, true, true, true);
			renderer.glBlendFunc(SGL.GL_ONE, SGL.GL_ONE_MINUS_SRC_COLOR);
		}
		postdraw();
	}
	...
	public void setBackground(Color color) {
		predraw();
		renderer.glClearColor(color.r, color.g, color.b, color.a);
		postdraw();
	}
	...
	public Color getBackground() {
		...
		renderer.glGetFloat(SGL.GL_COLOR_CLEAR_VALUE, buffer);
		postdraw();
	...
	public void clear() {
		predraw();
		renderer.glClear(SGL.GL_COLOR_BUFFER_BIT);
		postdraw();
	}
	...
	public void clearClip() {
		clip = null;
		predraw();
		renderer.glDisable(SGL.GL_SCISSOR_TEST);
		postdraw();
	}
	...
	public void setWorldClip(float x, float y, float width, float height) {
		...
		renderer.glEnable(SGL.GL_CLIP_PLANE0);
		worldClip.put(1).put(0).put(0).put(-x).flip();
		renderer.glClipPlane(SGL.GL_CLIP_PLANE0, worldClip);
		renderer.glEnable(SGL.GL_CLIP_PLANE1);
		worldClip.put(-1).put(0).put(0).put(x + width).flip();
		renderer.glClipPlane(SGL.GL_CLIP_PLANE1, worldClip);

		renderer.glEnable(SGL.GL_CLIP_PLANE2);
		worldClip.put(0).put(1).put(0).put(-y).flip();
		renderer.glClipPlane(SGL.GL_CLIP_PLANE2, worldClip);
		renderer.glEnable(SGL.GL_CLIP_PLANE3);
		worldClip.put(0).put(-1).put(0).put(y + height).flip();
		renderer.glClipPlane(SGL.GL_CLIP_PLANE3, worldClip);
		postdraw();
	}
	...
	public void clearWorldClip() {
		...
		renderer.glDisable(SGL.GL_CLIP_PLANE0);
		renderer.glDisable(SGL.GL_CLIP_PLANE1);
		renderer.glDisable(SGL.GL_CLIP_PLANE2);
		renderer.glDisable(SGL.GL_CLIP_PLANE3);
		postdraw();
	}
	...
	public void setClip(int x, int y, int width, int height) {
		...
		if (clip == null) {
			renderer.glEnable(SGL.GL_SCISSOR_TEST);
			clip = new Rectangle(x, y, width, height);
		} else {
			clip.setBounds(x,y,width,height);
		}
		
		renderer.glScissor(x, screenHeight - y - height, width, height);
		postdraw();
	}
	...
	public void fillRect(float x1, float y1, float width, float height) {
		...
		renderer.glVertex2f(x1, y1);
		renderer.glVertex2f(x1 + width, y1);
		renderer.glVertex2f(x1 + width, y1 + height);
		renderer.glVertex2f(x1, y1 + height);
		renderer.glEnd();
		postdraw();
	}
	...
	public void fillArc(float x1, float y1, float width, float height,
		...
		renderer.glBegin(SGL.GL_TRIANGLE_FAN);
		int step = 360 / segments;

		renderer.glVertex2f(cx, cy);
	...
	public void resetLineWidth() {
		...
		renderer.glLineWidth(1.0f);
		renderer.glPointSize(1.0f);
	...
	public void setAntiAlias(boolean anti) {
		...
		if (anti) {
			renderer.glEnable(SGL.GL_POLYGON_SMOOTH);
		} else {
			renderer.glDisable(SGL.GL_POLYGON_SMOOTH);
		}
		postdraw();
	}
	...
	public void copyArea(Image target, int x, int y) {
		...
		target.bind();
		renderer.glCopyTexImage2D(SGL.GL_TEXTURE_2D, 0, format, x, screenHeight
	...
	public Color getPixel(int x, int y) {
		predraw();
		renderer.glReadPixels(x, screenHeight - y, 1, 1, SGL.GL_RGBA,
	...
	public void getArea(int x, int y, int width, int height, ByteBuffer target)
	{
		...
		predraw();	
		renderer.glReadPixels(x, screenHeight - y - height, width, height, SGL.GL_RGBA,
	...
	public void drawGradientLine(float x1, float y1, float red1, float green1,
		...
		renderer.glBegin(SGL.GL_LINES);

		renderer.glColor4f(red1, green1, blue1, alpha1);
		renderer.glVertex2f(x1, y1);

		renderer.glColor4f(red2, green2, blue2, alpha2);
		renderer.glVertex2f(x2, y2);
	...
	public void pushTransform() {
		...
		renderer.glGetFloat(SGL.GL_MODELVIEW_MATRIX, buffer);
		buffer.put(16, sx);
		buffer.put(17, sy);
		stackIndex++;
	...
	public void popTransform() {
		...
		renderer.glLoadMatrix(oldBuffer);
		sx = oldBuffer.get(16);
		sy = oldBuffer.get(17);
